        ...
        for i, (option_value, option_label) in enumerate(chain(self.choices, choices)):
            ...
            if type(option_label) is list:
                for j, die_face in enumerate(option_label):
                    if has_id:
                        final_attrs = dict(
                            final_attrs, id="%s_%s_%s" % (attrs["id"], i, j)
                        )
                        label_for = format_html(' for="{0}"', final_attrs["id"])
                    else:
                        label_for = ""
                    if not (
                        type(option_value) is bool or not option_value or value == ""
                    ):
                ...
                if has_id:
                    ...
                    label_for = ""
                if not (
                    option_value is True
                    or option_value is False
...
class ChooseDiceForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["given_dice"].choices = self.make_choices_from_dict(given_dict)

    def make_choices_from_dict(self, die_count_dict):
        d_list = [key for key, value in die_count_dict.items() for i in range(0, value)]
...
class GatherDiceForm(forms.Form):
    dice_pool = forms.ChoiceField(label="Gather Your Die", widget=RadioImgWidget())
        ...
        self.fields["dice_pool"].choices = tuple(
            (
                "{resource} {face}_{count}".format(
                    resource=resource, face=die_face_tuple[0], count=die_face_tuple[1]
                ),
                "{count} {face}".format(
                    count=die_face_tuple[1], face=die_face_tuple[0].capitalize()
                ),
            )
